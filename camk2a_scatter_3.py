...
#dir_in = 'C:\\Users\\haley\\Dropbox\\Projects\\Connexin36 and Shank3\\Connexin36 KO'
dir_in = 'C:\\Users\\haley\\Dropbox\\Projects\\camk2a' # Directory on another computer
...
def get_frames (file_in, groups, vm_filter):
    ...
    df_group1 = df_group1[df_group1.vm1 <= vm_filter]
    df_group2 = df_group2[df_group2.vm1 <= vm_filter]
    df_group3 = df_group3[df_group3.vm1 <= vm_filter]
    df_groups = [df_group1, df_group2, df_group3, df_all]
...
def get_sig (df_groups, measure, repeated, groups, dir_out, exp_name):
    ...
    res_table = result._results_table
    q = result.q_crit
    rs = result.meandiffs/result.std_pairs
    p = psturng(np.abs(rs), len(result.groupsunique), result.df_total) 
    ...
    file_anova = exp_name + '_' + measure + '_anova.csv'
    save_csv (df_out, file_anova, dir_out, False)
    
    return f_value, p_anova, freedom, stars_anova, df_pairs 
    

def get_measure (df_groups, df_stats1, df_stats2, df_stats3, measure, repeated, exp_name, dir_out, groups):
    ...
    save_fig = exp_name + '_' + measure +'.jpg'         # Name of the output figure
    ...
    save_csv (df_stats, measure_out, dir_out, False) # False = Keep row labels
    
    # Run stats on two groups using independent t-test
    f_value, p_values, freedom, stars_anova, df_pairs = get_sig (df_groups, measure, repeated, groups, dir_out, exp_name)
        
    # Plot the data with the significance
    get_plot (df_groups, df_stats, y_column_name, save_fig, y_label, stars_anova, df_pairs, y_hline)
    
    return col_num

def get_plot (df_groups, df_stats, y_column_name, save_fig, y_label, stars_anova, df_pairs, y_hline):
    ...
    df_temp = df_groups[3].groupby('strain')
    y_minmax = df_temp[measure].max().min() 
    for index, row in df_pairs.iterrows():   
        if str(row['group1']) == 'cre' and str(row['group2']) == 'ko' and str(row['significance']) != 'n.s.':  
            row['significance'] = row['significance'].replace('*', '#')
            x_adj = get_x_adj (row['significance'])
            y_adj = y_minmax + 0.3 * y_minmax
            ax.text(x_max-1.5 + x_adj, y_adj, row['significance'], size = 12, color = 'black', fontweight = 'bold')
        if str(row['group1']) == 'ko' and str(row['group2']) == 'wt' and str(row['significance']) != 'n.s.':
            x_adj = get_x_adj (row['significance'])
            y_adj = y_minmax + 0.7 * y_minmax
            ax.text(x_max-1.5 + x_adj, y_adj, row['significance'], size = 18, color = 'black', fontweight = 'bold')
...
df_groups = get_frames (file_in, groups, vm_filter)
...
save_csv (df_stats1, save_group1, dir_out, True) # Ignore index = true (no row labels)
save_csv (df_stats2, save_group2, dir_out, True) # Ignore indec - True (no row labels)
save_csv (df_stats3, save_group3, dir_out, True) # Ignore indec - True (no row labels)


# Gets plots for each measurement
for measure in measures:
    col_num = get_measure(df_groups, df_stats1, df_stats2, df_stats3, 
                          measure, repeated, exp_name, dir_out, groups)
